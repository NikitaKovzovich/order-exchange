...
Тест проверяет корректную работу метода `login()`. Используются моки для репозитория пользователей, кодировщика паролей и провайдера JWT. Проверяется, что при валидных учетных данных возвращается JWT-токен.
...
Тест проверяет, что при неверном пароле выбрасывается исключение. Также проверяется, что последующие операции не выполняются.
...
Тест проверяет создание нового чат-канала для заказа. Проверяется, что канал сохраняется в репозиторий и возвращается корректный ответ.
...
Тест проверяет контроль доступа к чат-каналу. Пользователь, не являющийся участником чата, должен получить исключение AccessDeniedException.
...
Тесты проверяют корректность генерации и валидации JWT-токенов. Проверяется извлечение email и роли из токена, а также отклонение невалидных токенов.
